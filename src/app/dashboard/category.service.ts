import { Injectable } from '@angular/core';
import { HttpClient,HttpHeaders } from '@angular/common/http'; 
import { CategoryModel } from '../category-model';

@Injectable({
  providedIn: 'root'
})

export class CategoryService {
 

  constructor(private httpClient: HttpClient) { }

  httpOp:any;

  public getAllCategory()
  {
    return this.httpClient
			.get<any>('https://localhost:7043/api/category/getcategory',{headers: new HttpHeaders({ 
        'Content-Type': 'application/json',
        'Authorization': 'Bearer ' + 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..n4s7oCH105xo8Zz-6tGwCjgNknunOVvST3o3X7s7Q3o'
     })});
		
  }


  public postCategory(cate:CategoryModel){
 
    console.log(name);
    return this.httpClient.post<any>('https://localhost:7043/api/Category/Test1', cate,{headers: new HttpHeaders({ 
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..n4s7oCH105xo8Zz-6tGwCjgNknunOVvST3o3X7s7Q3o'
   })});
  }


  deleteCategory(id:number){
 
    console.log(id);
    return this.httpClient.post<any>('https://localhost:7043/api/category/test1', id,{headers: new HttpHeaders({ 
      'Content-Type': 'application/json',
      'Authorization': 'Bearer ' + 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..n4s7oCH105xo8Zz-6tGwCjgNknunOVvST3o3X7s7Q3o'
   })});
  }



}
